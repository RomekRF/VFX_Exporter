...
# ---- optional bake to origin (GLOBAL, animation-safe) ----
# Rotates everything by the root correction quaternion (same as __VFX_ROOT__) and then
# shifts the whole scene so it sits on Z=0 and is centered on X/Y.
#
# This is important for morph animation: the offset MUST be applied to *all* frames,
# not just frame0, or the mesh will "orbit" wildly.
def bake_origin_global(meshes: List['MeshOut'], root_q=(0.7071067811865476, 0.0, 0.0, 0.7071067811865476)) -> Tuple[float, float, float]:
    ...
    # 1) rotate everything into final Blender space (so we can compute a sane global offset)
    for m in meshes:
        m.verts = [tuple(_qrot(rq, v)) for v in m.verts]
        if m.frames_pos is not None:
            m.frames_pos = [[tuple(_qrot(rq, v)) for v in fr] for fr in m.frames_pos]

        if m.translation is not None:
            m.translation = tuple(_qrot(rq, m.translation))
        if m.trans_keys:
            m.trans_keys = [(t, tuple(_qrot(rq, p))) for (t, p) in m.trans_keys]

        # We keep rotations as-is; after this bake we will zero the root rotation in glTF.

    # 2) compute global bounds (frame0 geometry only)
    minx=miny=minz=1e30
    maxx=maxy=maxz=-1e30
    anyv=False
    for m in meshes:
    ...
    # 3) subtract offset from everything (including ALL morph frames!)
    ox, oy, oz = off
    for m in meshes:
        m.verts = [(x-ox, y-oy, z-oz) for (x,y,z) in m.verts]
        if m.frames_pos is not None:
            m.frames_pos = [[(x-ox, y-oy, z-oz) for (x,y,z) in fr] for fr in m.frames_pos]
        if m.translation is not None:
            tx,ty,tz = m.translation
            m.translation = (tx-ox, ty-oy, tz-oz)
        if m.trans_keys:
            m.trans_keys = [(t, (p[0]-ox, p[1]-oy, p[2]-oz)) for (t,p) in m.trans_keys]
...
def _gltf_pack_scene(
    ...
    root_node = {
        ...
        "rotation": ([0.7071067811865476, 0.0, 0.0, 0.7071067811865476] if root_rotation else [0.0, 0.0, 0.0, 1.0])
    }
    ...
    if center_geom:
        try:
            ...
            if acc0.get("type") == "VEC3":
                if "min" in acc0: acc0["min"] = [float(acc0["min"][0]) - cx, float(acc0["min"][1]) - cy, float(acc0["min"][2]) - cz]
                if "max" in acc0: acc0["max"] = [float(acc0["max"][0]) - cx, float(acc0["max"][1]) - cy, float(acc0["max"][2]) - cz]
...
def write_gltf_scene(path_gltf: str, meshes: List[MeshOut], materials: List[MaterialOut], scale: float = 10000.0, trs_scale: float = 1.0, center_root: bool = False, center_geom: bool = False, bake_origin: bool = False, root_rotation: bool = True) -> None:
    out_dir = os.path.dirname(path_gltf) or "."
    base_name = os.path.splitext(os.path.basename(path_gltf))[0]
    out_gltf, out_bin = _gltf_pack_scene(meshes, materials, out_dir, base_name, scale=scale, trs_scale=trs_scale, center_root=center_root, center_geom=center_geom, bake_origin=bake_origin, root_rotation=root_rotation)
...
def convert_file(path_vfx: str, scale: float = 10000.0, trs_scale: float = 1.0, center_root: bool = False, center_geom: bool = False, bake_origin: bool = False, bake_origin_global_on: bool = True) -> None:
    ...
    if EXPORT_GLTF:
        # Default: bake a global origin offset (XY centered, Z floored) in a morph-safe way.
        # Disable with --keep-world.
        root_rotation = True
        if bake_origin_global_on:
            _ = bake_origin_global(meshes)
            root_rotation = False  # we already rotated vertices/keys
        write_gltf_scene(base + '.gltf', meshes, materials, scale=scale, trs_scale=trs_scale, center_root=center_root, center_geom=center_geom, bake_origin=bake_origin, root_rotation=root_rotation)
    for m in meshes:
...
def main(argv: List[str]) -> int:
    ...
    bake_origin_global_on = True  # default for --gltf/--both; can disable with --keep-world
    ...
    for p in args:
        if os.path.isfile(p):
            try:
                convert_file(p, scale=scale, trs_scale=trs_scale, center_root=center_root, center_geom=center_geom, bake_origin=bake_origin, bake_origin_global_on=bake_origin_global_on)
            except Exception as e:
                print(f"[ERROR] {p}: {e}")
        else:
