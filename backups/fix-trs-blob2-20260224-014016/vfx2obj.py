...
def main(argv: List[str]) -> int:
    ...
    while i < len(args):
        ...
        if a in ('--debug-frames',):
            ...
            del args[i:i+1]
            continue
        if a in ('--trs-scale',):             if i + 1 >= len(args): raise SystemExit('Missing value after --trs-scale')             trs_scale = float(args[i + 1])             i += 1             continue         if a.startswith('--trs-scale='):             trs_scale = float(a.split('=', 1)[1])             continue
        if a in ('--scale', '-s'):
            if i + 1 >= len(args):
