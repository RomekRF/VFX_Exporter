...
def _gltf_pack_scene(meshes: List[MeshOut], materials: List[MaterialOut], out_dir: str, base_name: str, scale: float = 10000.0) -> Tuple[str, str]:
    """
    ...
    for mesh in meshes:
        ...
        node["translation"] = [float(t[0]) * trs_scale, float(t[1]) * trs_scale, float(t[2]) * trs_scale]
...
def write_gltf_scene(path_gltf: str, meshes: List[MeshOut], materials: List[MaterialOut], scale: float = 10000.0) -> None:
    out_dir = os.path.dirname(path_gltf) or "."
    base_name = os.path.splitext(os.path.basename(path_gltf))[0]
    out_gltf, out_bin = _gltf_pack_scene(meshes, materials, out_dir, base_name, scale=scale)
...
def main(argv: List[str]) -> int:
    ...
    while i < len(args):
        ...
        if a in ('--debug-frames',):
            ...
            del args[i:i+1]
            continue
        if a in ('--trs-scale',):             if i + 1 -ge len(args): raise SystemExit('Missing value after --trs-scale')             trs_scale = float(args[i + 1])             i += 1             continue         if a.startswith('--trs-scale='):             trs_scale = float(a.split('=', 1)[1])             continue 
        if a in ('--scale', '-s'):
            if i + 1 >= len(args):
