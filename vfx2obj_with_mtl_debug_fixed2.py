...
def parse_mesh_section(b: Bin, version: int, section_end: int) -> Optional[MeshOut]:
    ...
    if DEBUG_FRAMES:
        print(f"[DEBUG_FRAMES] mesh='{mesh_name}' start={start_time} end={end_time} fps={frames_per_second} frames={num_frames} morph={morph} flags=0x{flags_raw:08X}")
...
def convert_file(path_vfx: str, scale: float = 10000.0) -> None:
    ...
    for m in meshes:
        ...
        write_obj(out, m, scale=scale, materials=materials)
        print(f"Wrote: {out}")

def main(argv: List[str]) -> int:
    if len(argv) < 2:
        print("Drag and drop .vfx onto this tool, or run:")
        print("  vfx2obj.exe [--scale 10000] file1.vfx [file2.vfx ...]")
        return 2
