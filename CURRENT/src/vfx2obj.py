...
def parse_mesh_section(b: Bin, version: int, section_end: int) -> Optional[MeshOut]:
    ...
    for frame_idx in range(num_frames):
        ...
        if morph or frame_idx == 0:
            ...
            # Decompress into a per-frame array (length == num_vertices)
            ...
            for _ in range(num_vertices):
                ...
                frame_verts.append(rf_to_blender((vx, vy, vz)))

            # Keep frame 0 as the base static mesh
            if frame_idx == 0:
                base_verts = frame_verts
                verts = base_verts

            # If capturing morph animation, store per-frame positions (independent lists)
            if frames_pos is not None:
                frames_pos.append(frame_verts)
        ...
        # For non-morph frames > 0, positions are not present (only UV/transform blocks follow)
...
def _gltf_pack_scene(meshes: List[MeshOut], materials: List[MaterialOut], out_dir: str, base_name: str, scale: float = 10000.0, trs_scale: float = 1.0, center_root: bool = False, center_geom: bool = False, bake_origin: bool = False) -> Tuple[str, str]:
    ...
    root_node = {'name': '__VFX_ROOT__', 'children': scene_nodes, 'rotation': [0.0, 0.0, 0.0, 1.0]}
